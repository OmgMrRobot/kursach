...
# Fd = 4*(fL1 +16*pow(10,6))

# Tmax = 1000*1/

# # 1,023 Мгц 16Мгц - полоса
...
sequence = G1_G2_G2i(n)[1]  # получаем нашу м-последовательность 
sequence = adapter(sequence)  # меняем 0 на -1

#не работает 
sig = [sin(2.*pi*fL1*t) for t in range(n)] # создаем сигнал

modul_sig = [sig[i]*sequence[i] for i in  range(n)] # Модулируем сигнал нашей м-последовательностью


# вычисляем преобразование Фурье. Сигнал действительный, поэтому надо использовать rfft
spectrum = rfft(modul_sig)

def plotTime(n, FD, modul_sig):
	# нарисуем всё это, используя matplotlib
	# Сначала сигнал зашумлённый и тон отдельно
	plt.plot(arange(n)/float(FD), modul_sig) # по оси времени секунды!
...
def plotSpectrum(n , FD, spectrum):
	# Потом спектр
	plt.plot(rfftfreq(n, 1./FD), absolute(spectrum)/n)

	plt.axis([fL1-pow(10,8) , fL1+pow(10,8), 0, pow(10,-3)]) # пределы по осям xmin xmax yman ymax
