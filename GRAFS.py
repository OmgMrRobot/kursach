...
def ModulatedSig(sig, sequence, Code):# Модулирует сигнал м-последовательностью
	modulsig = []
	for i in range(len(sequence)):
...
def Sequence(sequence, Code): # дублирует значения м-последовательности 
	sequence2 = []
	for i in range(len(sequence)):
		for j in range(len(Code)):
			if i == Code[j]:
...
def Grafs(equal):
	...
	sequence = Gold(equal)  # получаем нашу м-последовательность 
